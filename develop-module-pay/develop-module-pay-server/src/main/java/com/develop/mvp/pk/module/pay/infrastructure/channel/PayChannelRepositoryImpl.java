...
public class PayChannelRepositoryImpl implements PayChannelRepository {
    @Resource private PayChannelMapper mapper;
    static PayChannel toDomain(PayChannelDO doObj) {
        if (doObj == null) return null;
        return new PayChannel(doObj.getId(), doObj.getCode(), doObj.getAppId())
                .status(doObj.getStatus()).feeRate(doObj.getFeeRate())
                .remark(doObj.getRemark()).config(doObj.getConfig());
    }
    static PayChannelDO toDO(PayChannel domain) {
        PayChannelDO doObj = new PayChannelDO();
        doObj.setId(domain.id()); doObj.setCode(domain.code()); doObj.setAppId(domain.appId());
        doObj.setStatus(domain.status()); doObj.setFeeRate(domain.feeRate());
        doObj.setRemark(domain.remark()); doObj.setConfig((com.develop.mvp.pk.module.pay.framework.pay.core.client.PayClientConfig) domain.config());
        return doObj;
    }
    @Override public PayChannel save(PayChannel channel) {
        if (channel.id() == null) { mapper.insert(toDO(channel)); return channel; }
        mapper.updateById(toDO(channel)); return channel;
    }
    @Override public PayChannel findById(Long id) { return toDomain(mapper.selectById(id)); }
    @Override public Optional<PayChannel> findByAppIdAndCode(Long appId, String code) {
        return Optional.ofNullable(toDomain(mapper.selectByAppIdAndCode(appId, code)));
    }
    @Override public List<PayChannel> findByAppIds(Collection<Long> appIds) {
        return mapper.selectListByAppIds(appIds).stream().map(PayChannelRepositoryImpl::toDomain).collect(Collectors.toList());
    }
    @Override public List<PayChannel> findEnabledByAppId(Long appId) {
