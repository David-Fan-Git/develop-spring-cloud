...
public class PayAppRepositoryImpl implements PayAppRepository {
    @Resource private PayAppMapper mapper;
    static PayApp toDomain(PayAppDO doObj) {
        if (doObj == null) return null;
        return new PayApp(doObj.getId(), doObj.getName())
                .appKey(doObj.getAppKey()).status(doObj.getStatus()).remark(doObj.getRemark())
                .orderNotifyUrl(doObj.getOrderNotifyUrl())
                .refundNotifyUrl(doObj.getRefundNotifyUrl())
                .transferNotifyUrl(doObj.getTransferNotifyUrl());
    }
    static PayAppDO toDO(PayApp domain) {
        return PayAppDO.builder().id(domain.id()).appKey(domain.appKey()).name(domain.name())
                .status(domain.status()).remark(domain.remark())
                .orderNotifyUrl(domain.orderNotifyUrl())
                .refundNotifyUrl(domain.refundNotifyUrl())
                .transferNotifyUrl(domain.transferNotifyUrl()).build();
    }
    @Override public PayApp save(PayApp app) {
        if (app.id() == null) { mapper.insert(toDO(app)); return app; }
        mapper.updateById(toDO(app)); return app;
    }
    @Override public PayApp findById(Long id) { return toDomain(mapper.selectById(id)); }
    @Override public Optional<PayApp> findByAppKey(String appKey) { return Optional.ofNullable(toDomain(mapper.selectByAppKey(appKey))); }
    ...
    @Override public boolean existsById(Long id) { return mapper.selectById(id) != null; }
}
