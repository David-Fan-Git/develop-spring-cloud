...
物联网模块，提供产品、设备、物模型、设备消息、协议网关、设备接入等 IoT 能力。
...
| Maven Artifact | `develop-module-iot` |
| Packaging | `pom` |
| Java 源文件数量 | 435 |
| 模块说明 | 物联网模块 |
...
| 子模块 | 职责 |
|---|---|
| `develop-module-iot-api` | 跨模块契约、DTO、枚举、CommonApi / RPC API。 |
| `develop-module-iot-core` | 核心抽象、公共模型或跨运行单元复用能力。 |
| `develop-module-iot-server` | 业务实现、Controller、Service/ApplicationService、Domain、Infrastructure、DAL、MQ、Job。 |
| `develop-module-iot-gateway` | 独立网关或协议接入运行单元。 |
...
```bash
# 编译该聚合模块及其子模块
mvn compile -pl develop-module-iot -am

# 打包该聚合模块及其子模块
mvn clean package -pl develop-module-iot -am -Dmaven.test.skip=true
```
